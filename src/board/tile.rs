...
impl Widget for Tile {
    fn render(&self, buffer: &mut Buffer) {
        let db = self.color();
        let mut tbuffer =
            buffer.get_subset(Rect::new(buffer.x(), buffer.y(), 6, 3));

        tbuffer.set_str_styled(
            " ▆▆▆▆▆ ",
            &tbuffer.pos(),
            Style::new().fg(Color::Hex(db)).bg(Color::Hex(0x797979)),
        );
        tbuffer.set_str_styled(
            self.value_str(),
            &Coords::new(buffer.x() + 1, buffer.y() + 1),
            Style::new().bg(Color::Hex(db)).fg(Color::Black),
        );
        tbuffer.set_str_styled(
            " ",
            &Coords::new(buffer.x(), buffer.y() + 2),
            Style::new().bg(Color::Hex(0x797979)),
        );
        tbuffer.set_str_styled(
            "▂▂▂▂▂",
            &Coords::new(buffer.x() + 1, buffer.y() + 2),
            Style::new().bg(Color::Hex(db)).fg(Color::Hex(0x797979)),
        );
...
impl Into<Box<dyn Widget>> for Tile {
